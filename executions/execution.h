...
void Damage_Anim(bool eop,double hp,double initial_hp,int initial_damage) {
	...
	DestRect.w = ceil(((double)Parties[eop].Member[0]->CurrentHp/Parties[eop].Member[0]->Hp)*48);
		...
		while (Anim_running) {
			...
			// Draw
			DestRect.w = ceil(((double)initial_hp/Parties[eop].Member[0]->Hp)*48);
			SDL_SetRenderDrawColor(renderer,32,32,32,255);
			SDL_RenderFillRect(renderer,&SourceRect);
			SDL_SetRenderDrawColor(renderer,108,108,120,255);
			SDL_RenderFillRect(renderer,&((const SDL_Rect){46+(!eop*152),49+(!eop*76),20,8}));
			sprintf(TempTextBuffer2,"%.0f",initial_hp);
			drawText(TempTextBuffer2,66+(!eop*152)-(strlen(TempTextBuffer2)*5),49+(!eop*76),255,255,255,SMALL_FONT);
...
void UDBOG(int* hp,int* damage) {
  ...
  *hp -= *damage;
	Damage_Anim((hp == &Parties[1].Member[0]->CurrentHp),(double)(*hp),(double)initial_hp,initial_damage);
}
...
void ExecuteMove(bool eop) {
    if (!Parties[eop].Dead) {
        ...
        MOVE_FUNC_LIST[MoveList[Parties[eop].Turn->Move].movefunc & REMOVE_FIRST_FIVE_BITS](3,eop,1);
        display_move(eop);
      if (Parties[eop].Member[0]->CurrentHp > 0 && Parties[eop].Hit && Parties[eop].CanMove) {
        ...
        ABILITY_FUNC_LIST[AbilityList[Parties[eop].Member[0]->Ability].abilityfunc](2,eop);
        ABILITY_FUNC_LIST[AbilityList[Parties[!eop].Member[0]->Ability].abilityfunc](-4,!eop);
        Parties[eop].Damage = never0(((((2 * Parties[eop].Member[0]->Level / 5 + 2) * (Parties[eop].Member[0]->Atk*(Parties[eop].Crit && (Parties[eop].Member[0]->StatBoosts[0] < 0) ? 1 : statboostmult(Parties[eop].Member[0]->StatBoosts[0]))) * MoveList[Parties[eop].Turn->Move].BP / (Parties[!eop].Member[0]->Def*(Parties[eop].Crit && (Parties[!eop].Member[0]->StatBoosts[1] > 0) ? 1 : statboostmult(Parties[!eop].Member[0]->StatBoosts[1])))) / 50) + 2) * (Parties[eop].STAB *= (Parties[eop].MoveTempType == POKEMONDEX[Parties[eop].Member[0]->Poke].Type1 || Parties[eop].MoveTempType == POKEMONDEX[Parties[eop].Member[0]->Poke].Type2 ? 1.5 : 1)) * (TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type1] * TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type2]) * ((rand() % 16) + 85) / 100 * Parties[eop].TemporaryMult * (Parties[eop].Crit ? 1.5 : 1));
        MOVE_FUNC_LIST[MoveList[Parties[eop].Turn->Move].movefunc >> 5](1,eop,0);
  ...
  if (!(CHK_BIT(Parties[!eop].EFFECT_FLAGS[0],EFFECT_PROTECT) && CHK_BIT(MoveList[Parties[eop].Turn->Move].FLAGS,nFLAG_PROTECT_AFFECTED))) {
    ...
    Parties[eop].Damage = 0;
        ...
        ABILITY_FUNC_LIST[AbilityList[Parties[eop].Member[0]->Ability].abilityfunc](2,eop);
        ABILITY_FUNC_LIST[AbilityList[Parties[!eop].Member[0]->Ability].abilityfunc](-4,!eop);
        Parties[eop].Damage = never0((((((2 * Parties[eop].Member[0]->Level / 5 + 2) * (Parties[eop].Member[0]->SpA*(Parties[eop].Crit && (Parties[eop].Member[0]->StatBoosts[2] < 0) ? 1 : statboostmult(Parties[eop].Member[0]->StatBoosts[2]))) * MoveList[Parties[eop].Turn->Move].BP / (Parties[!eop].Member[0]->SpD*(Parties[eop].Crit && (Parties[!eop].Member[0]->StatBoosts[3] > 0) ? 1 : statboostmult(Parties[!eop].Member[0]->StatBoosts[3])))) / 50) + 2) * (Parties[eop].STAB *= (Parties[eop].MoveTempType == POKEMONDEX[Parties[eop].Member[0]->Poke].Type1 || Parties[eop].MoveTempType == POKEMONDEX[Parties[eop].Member[0]->Poke].Type2 ? 1.5 : 1)) * TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type1] * TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type2] * ((rand() % 16) + 85) / 100) * Parties[eop].TemporaryMult * (Parties[eop].Crit ? 1.5 : 1));
        MOVE_FUNC_LIST[MoveList[Parties[eop].Turn->Move].movefunc >> 5](1,eop,0);
        MOVE_FUNC_LIST[MoveList[Parties[eop].Turn->Move].movefunc & REMOVE_FIRST_FIVE_BITS](1,eop,1);
        ITEM_FUNC_LIST[ItemList[Parties[eop].Member[0]->Item].itemfunc](1,eop);
        ITEM_FUNC_LIST[ItemList[Parties[!eop].Member[0]->Item].itemfunc](-1,!eop);
