void TextBoxUpdate(char* text,unsigned int character_delay,unsigned int pause) {
	...
	while(Updating) {
        ...
        SDL_RenderFillRect(renderer,&DestRect);
				...
				drawText(TempTextBuffer2,218-(strlen(TempTextBuffer2)*5),125,255,255,255,SMALL_FONT);

				sprintf(TempTextBuffer2,"%d",Parties[0].Member[0]->Hp);
				drawText(TempTextBuffer2,225,125,255,255,255,SMALL_FONT);

				drawText(str_decompress_format_prealloc(POKEMONDEX[Parties[0].Member[0]->Poke].Name,TempTextBuffer2),154,103,255,255,255,REGULAR_FONT);
        ...
        SDL_RenderFillRect(renderer,&DestRect);
				...
				drawText(TempTextBuffer2,66-(strlen(TempTextBuffer2)*5),49,255,255,255,SMALL_FONT);

				sprintf(TempTextBuffer2,"%d",Parties[1].Member[0]->Hp);
				drawText(TempTextBuffer2,73,49,255,255,255,SMALL_FONT);

				drawText(str_decompress_format_prealloc(POKEMONDEX[Parties[1].Member[0]->Poke].Name,TempTextBuffer2),2,27,255,255,255,REGULAR_FONT);
        ...
        SDL_RenderCopy(renderer,TextBox,NULL,&DestRect);
				...
				if (!drawScrollingText(text, 12, 152, 0, 0, 0, REGULAR_FONT, character_delay)) {
...
void TextBoxUpdateFast(char* text,unsigned int character_delay,unsigned int pause) {
	...
	{	
		...
		if ((i*character_delay) > (SDL_GetTicks() - start_time)) SDL_Delay((i*character_delay) - (SDL_GetTicks() - start_time));
...
void display_move(bool eop) {
          ...
          sprintf(TempTextBuffer,"%s%s used %s!",EOPTEXT[eop],str_decompress_format_prealloc(POKEMONDEX[Parties[eop].Member[0]->Poke].Name,TempTextBuffer3),str_decompress_format_prealloc(MoveList[Parties[eop].Turn->Move].Name,TempTextBuffer2));
					TextBoxUpdateFast(TempTextBuffer,50,1000);
          if (!Parties[eop].Hit) TextBoxUpdateFast("But it missed!",50,1000);
          } else if (!Parties[eop].Confused) {
          sprintf(TempTextBuffer,"%s%s couldn't move",EOPTEXT[eop],TempTextBuffer3);
					TextBoxUpdateFast(TempTextBuffer,50,1000);
          if (Parties[eop].Flinch) TextBoxUpdateFast("It flinched!",50,1000);
          else if (Parties[eop].Para) TextBoxUpdateFast("It was paralyzed!",50,1000);
          else if (Parties[eop].Sleep) TextBoxUpdateFast("It is asleep",50,1000);
          else if (Parties[eop].Frozen) TextBoxUpdateFast("It is frozen!",50,1000);
          }
}
...
void move_result(bool eop) {
        ...
        if (TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type1] * TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type2] >= 2) {
          ...
          if (TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type1] * TypeChart[Parties[eop].MoveTempType][POKEMONDEX[Parties[!eop].Member[0]->Poke].Type2] <= 0) {
						sprintf(TempTextBuffer,"It dosen't affect %s",str_decompress_format_prealloc(POKEMONDEX[Parties[!eop].Member[0]->Poke].Name,TempTextBuffer2));
						TextBoxUpdateFast(TempTextBuffer,50,1000);
						}
          else TextBoxUpdateFast("It's not very effective...",50,1000);
          }
        if (Parties[eop].Crit && Parties[eop].Damage > 0) TextBoxUpdateFast("It was a critical hit!",50,1000);
          }
        else {
        if (CHK_BIT(Parties[!eop].EFFECT_FLAGS[0],EFFECT_PROTECT)) {
        sprintf(TempTextBuffer,"%s%s protected itself!",EOPTEXT[!eop],str_decompress_format_prealloc(POKEMONDEX[Parties[!eop].Member[0]->Poke].Name,TempTextBuffer2));
				TextBoxUpdateFast(TempTextBuffer,50,1000);
        } else {
        sprintf(TempTextBuffer,"It had no effect on %s%s...",EOPTEXT[!eop + 2],str_decompress_format_prealloc(POKEMONDEX[Parties[!eop].Member[0]->Poke].Name,TempTextBuffer2));
				TextBoxUpdateFast(TempTextBuffer,50,1000);
          }
          }
        ...
        if (Parties[!eop].Member[0]->CurrentHp <= 0) {
          DisplayDie(!eop);
          sprintf(TempTextBuffer,"%s%s fainted!",EOPTEXT[!eop],str_decompress_format_prealloc(POKEMONDEX[Parties[!eop].Member[0]->Poke].Name,TempTextBuffer2));
					TextBoxUpdateFast(TempTextBuffer,50,1000);
					SwitchIn(!eop);
          Parties[!eop].Dead = 1;
					}
