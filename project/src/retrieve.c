...
void RetrieveAIMove(bool eop)
{
	...
	bool WillSwitch = 0;
	while (1) {
		if (rand() % 10 || ((Parties[eop].Member[1]->CurrentHp > 0) +
							(Parties[eop].Member[2]->CurrentHp > 0) +
							(Parties[eop].Member[3]->CurrentHp > 0) +
							(Parties[eop].Member[4]->CurrentHp > 0) +
							(Parties[eop].Member[5]->CurrentHp > 0)) == 0) {
			aimove = rand() % 4;
		}
		else {
			aimove = 1 + (rand() % ((Parties[eop].Member[1]->CurrentHp > 0) +
									(Parties[eop].Member[2]->CurrentHp > 0) +
									(Parties[eop].Member[3]->CurrentHp > 0) +
									(Parties[eop].Member[4]->CurrentHp > 0) +
		...
		if (!WillSwitch) {
			...
			if (Parties[eop].Member[0]->Moves[aimove].Move == Nothing)
				continue;
			if (Parties[eop].Member[0]->Moves[0].PP &&
				Parties[eop].Member[0]->Moves[1].PP <= 0 &&
				Parties[eop].Member[0]->Moves[2].PP <= 0 &&
				Parties[eop].Member[0]->Moves[3].PP <= 0) {
				Parties[eop].Turn = &Struggle_Slot;
				break;
			}
			if (Parties[eop].Member[0]->Moves[aimove].PP <= 0)
				continue;
			break;
		}
		else {
			if (Parties[eop].Member[aimove] == NULL ||
				Parties[eop].Member[aimove]->CurrentHp <= 0)
				continue;
...
void RetrieveUserMove(bool eop)
{
	...
	while (!Done) {
		while (Retrieving) {
			...
			drawText(TempTextBuffer, 218 - (strlen(TempTextBuffer) * 5),
					 125 + !UpAndDown, 255, 255, 255, SMALL_FONT);

			sprintf(TempTextBuffer, "%d", Parties[0].Member[0]->Hp);
			drawText(TempTextBuffer, 225, 125 + !UpAndDown, 255, 255, 255,
					 SMALL_FONT);

			drawText(POKEMONDEX[Parties[0].Member[0]->Poke].Name, 154,
					 103 + !UpAndDown, 255, 255, 255, REGULAR_FONT);

			sprintf(TempTextBuffer, "%d", Parties[0].Member[0]->Level);
			drawText(TempTextBuffer, 234, 104 + !UpAndDown, 255, 255, 255,
					 SECONDARY_FONT);
			...
			DestRect.w = 128;
			DestRect.h = 42;
			SDL_RenderCopy(renderer, EnemyHpBar, NULL,
						   &DestRect); // 50,25 | 97,26
			...
			drawText(TempTextBuffer, 66 - (strlen(TempTextBuffer) * 5), 49, 255,
					 255, 255, SMALL_FONT);

			sprintf(TempTextBuffer, "%d", Parties[1].Member[0]->Hp);
			drawText(TempTextBuffer, 73, 49, 255, 255, 255, SMALL_FONT);

			drawText(POKEMONDEX[Parties[1].Member[0]->Poke].Name, 2, 27, 255,
			...
			SDL_RenderCopy(renderer,
						   Pokemon_Back_Sprites[Parties[0].Member[0]->Poke],
						   NULL, &DestRect);
			...
			if (!Menu) {
				...
				drawText(MoveList[Parties[eop].Member[0]->Moves[0].Move].Name,
						 16, 155, 0, 0, 0, SMALL_FONT);
				drawText(MoveList[Parties[eop].Member[0]->Moves[1].Move].Name,
						 105, 155, 0, 0, 0, SMALL_FONT);
				drawText(MoveList[Parties[eop].Member[0]->Moves[2].Move].Name,
						 16, 173, 0, 0, 0, SMALL_FONT);
				drawText(MoveList[Parties[eop].Member[0]->Moves[3].Move].Name,
						 105, 173, 0, 0, 0, SMALL_FONT);

				sprintf(
					TempTextBuffer, "PP: %d/%0.f",
					Parties[eop].Member[0]->Moves[OptionSelect].PP,
					(MoveList[Parties[eop].Member[0]->Moves[OptionSelect].Move]
						 .PP +
					 1) *
						5 *
						ppboostmult(Parties[eop]
										.Member[0]
										->Moves[OptionSelect]
										.PPmult));
