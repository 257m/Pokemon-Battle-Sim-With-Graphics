...
void TextBoxUpdate(char* text, unsigned int character_delay, unsigned int pause)
{
	...
	while (Updating) {
		...
		if (!drawScrollingText(text, 12, 152, 0, 0, 0, REGULAR_FONT,
							   character_delay)) {
...
void TextBoxUpdateFast(char* text, unsigned int character_delay,
					   unsigned int pause)
{
	// unsigned int last_frame,this_frame;
	// Draw
...
void display_move(bool eop)
{
	if (Parties[eop].CanMove) {
		sprintf(TempTextBuffer, "%s%s used %s!", EOPTEXT[eop],
				...
				MoveList[Parties[eop].Turn->Move].Name);
			...
			if (Parties[eop].Damage > 0) {
				...
				if (TypeChart[Parties[eop].MoveTempType]
					...
					if (TypeChart[Parties[eop].MoveTempType]
						...
						TextBoxUpdateFast("It's not very effective...", 50,
										  1000);
				}
