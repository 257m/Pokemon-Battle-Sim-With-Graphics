...
{
	if (et == EET_POSTMOVE) {
		if (((unsigned char)MoveList[Parties[eop].Turn->Move].movefunc >> 5 !=
				MF_PROTECT) &&
			((~MoveList[Parties[eop].Turn->Move].movefunc &
				 REMOVE_FIRST_FIVE_BITS) != MF_PROTECT))
...
{
	for (int i = 0;
		 i < (sizeof(EFFECT_FUNC_LIST) / sizeof(EFFECT_FUNC_LIST[0])); i++) {
