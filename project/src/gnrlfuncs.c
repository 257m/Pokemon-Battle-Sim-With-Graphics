...
float ppboostmult(char ppboost) { return 1 + (0.2 * ppboost); }
/*
...
unsigned int map2(
	unsigned int num, unsigned int newmax, unsigned int currentmax)
{
	if (currentmax == 0)
		currentmax = RAND_MAX;
	if (num > (currentmax - (currentmax % newmax)))
		return map2(rand(), newmax, currentmax);
	else
		return (num % newmax);
}
/*
these map function are now obsolete but they used to make random number into a
